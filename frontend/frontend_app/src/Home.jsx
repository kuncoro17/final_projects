...
const Home = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (action === 'masuk') {
                // Check-in: Fetch data
                response = await fetch(`http://localhost:3002/api/kartu-absen/by-nomor-kartu?nomor_kartu=${nomorKartu}`);
            } else if (action === 'pulang') {
                // Check-out: Update data
                response = await fetch(`http://localhost:3002/api/kartu-absen/pulang?nomor_kartu=${nomorKartu}`);
            }
            ...
            if (data) {
                if (action === 'masuk') {
                    ...
                    // Navigate to the checkout-page with the fetched data
                    navigate('/checkout-page', { state: { kartuAbsen: data } });
                }
            } else {
                    ...
                    <div className="container mx-auto bg-blue-700 p-10 rounded-lg shadow-2xl max-w-lg">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <select
                                ...
                                className="form-select w-full text-xl py-2 px-4 rounded-md border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
                            >
